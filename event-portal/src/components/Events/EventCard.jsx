...
const EventCard = ({ event, onWithdraw, onEdit, onDelete, myRegistration }) => {
  ...
  const addToCalendar = () => {
    ...
    const venue = (new Date(event.date) <= new Date() || user?.role !== 'Participant') ? event.venue : 'Location revealed at start';
    const url = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(event.title)}&dates=${start}/${end}&details=${encodeURIComponent(event.description)}&location=${encodeURIComponent(venue)}`;
