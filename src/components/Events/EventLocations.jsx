        ...
        {events.filter(e => new Date(e.date) >= new Date()).map((event, idx) => (
          ...
          >
            ...
            <div className="relative z-10">
              ...
              {new Date(event.date) <= new Date() || user?.role !== 'Participant' ? (
                <>
                ...
                <div className="flex flex-col items-center justify-center p-8 bg-black/40 border border-white/5 rounded-2xl border-dashed">
                  <div className="w-10 h-10 bg-indigo-500/10 rounded-full flex items-center justify-center mb-3">
                    <MapPinIcon className="w-5 h-5 text-indigo-500/50" />
                  </div>
                  <p className="text-xs font-black text-indigo-400 uppercase tracking-widest text-center">Location Encrypted</p>
                  <p className="text-[10px] text-gray-500 mt-1 text-center font-medium">Revealed at {new Date(event.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              )}
            </div>
          </motion.div>
